...
| 2026-09-21 | `4e7f3c58a4` | 修复 | **移植 LDS `os4-branch` 的面板休眠门控：修复熄屏后三个常驻循环空转导致的桌面耗电异常**（用户实测：自打包桌面耗电明显偏大）。<br>**来源**：`LDS-XiaoYe:os4-branch` 的 `06be304cb8`（引入面板门控，与 OS4 布局运行时同一个提交）与 `0f912b9970`（`fix(home): stop the dock sweep from ticking all night`）。**只摘与布局运行时无关的省电部分**；布局运行时（`home_layout_*` / `hometweaks` / `nhk::memory_io`）按既定方针不跟。<br>**成因**：`libHyperCeilerNative.so` 跑在 `com.miui.home` 进程里，耗电因此记在「桌面」名下。三个循环在屏幕熄灭后都不停：① `hc-dock-health` 线程 1s（不健康时 2s）一跳，判定为 `force / inventory_changed / !healthy` 任一成立即扫描 —— 只要「不健康」，每个 pass 都会触发**全量可执行镜像拷贝 + 模式扫描**（预算 64MB），launcher build 匹配不上时整天不停；② `hc-dock-motion` 线程 `poll` 超时 1000ms，每秒唤醒进程；③ 底栏 sweep（跑在 `system_server`）1s 起、空闲退避封顶 8s，每轮还重读底栏偏好与旋转门。<br>**改法**（4 文件，全部**纯插入**，不改任何功能判定）：`DockNativeMotionEndpoint.java` 新增 `STATE_TRANSACTION_CODE = 0x0048434D` / `STATE_ACK = 0x48435331`（`handles()` 只认原有两条 → before hook 天然放过未知码，透传即可）；`dock_native_motion.cpp` 新增 `dock_motion_screen_active()` / `refresh_dock_screen_state()`（同步 `IWindowManager` 事务向 system_server 询问面板状态 —— 背光节点与 DRM connector 都是 root-only，PowerManagerService 在 system_server），`poll` 超时按状态切 1000ms / 30s；`dock_native_hooks.cpp` 的 health worker 加面板门控（休眠 30s 一跳）+ 新增 `unresolved_retry_delay()` 退避阶梯（2s 起指数增长、上限 60s），把「匹配不上的 launcher build」从每 2 秒一次全镜像扫描降到每分钟一次；`HomeDockWindow.kt` 新增 `screenInteractive()`（复用缓存的 `PowerManager`，fail-open）、STATE 事务应答分支、sweep 的 30s 休眠门控。<br>**fail-open**：查询失败保持上一次状态；端点缺失只损失省电，不会停掉 hook。<br>**校验**：门控函数 / `health_worker` / `screenInteractive` / sweep 门控四处与 LDS 原文**逐字节一致**；事务码全树无冲突；无 `HomeLayoutNativeEndpointOS4` / `hometweaks` / `nhk::memory_io` 引用泄漏；tree 文件数 2075 不变、变更集恰好 4 个；`hc_verify_tree.py` 全项 PASS。 |
